...
func TestValidateDestinationExistsWithOptionalParams(t *testing.T) {
	t.Skip("TODO: investigate broken test")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Logpush.Validate.DestinationExists(context.TODO(), logpush.ValidateDestinationExistsParams{
		DestinationConf: cloudflare.F("s3://mybucket/logs?region=us-west-2"),
		AccountID:       cloudflare.F("account_id"),
	})
	if err != nil {
